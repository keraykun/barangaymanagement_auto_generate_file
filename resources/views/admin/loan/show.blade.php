...
<div class="mb-5" id="testMe">
    <div>
        <a class="py-2 px-3 bg-blue-500 text-white" href="{{ route('admin.residentlist.detail',$loan->id) }}">Back</a>
    </div>
    <div class="w-full flex items-center justify-center gap-3" >
            ...
            <div class="bg-white w-[100%] flex flex-col gap-5 py-3 px-1"  id="contentToPrint">
                <section class="flex justify-center items-center gap-3">
                    ...
                    <article class="text-lg text-center">
                        ...
                        <p>Province of {{ $loan->district->barangay->municipal->province->name }} Municipality of {{ $loan->district->barangay->municipal->name }}</p>
                        <p class="font-bold">BARANGAY {{ Str::upper($loan->district->barangay->name) }}</p>
                  ...
                  <div class="article-content flex gap-3">
                     ...
                     <div class="article-right" style="font-family:pacifico;">
                        <section class="text-center flex flex-col gap-4  font-bold  items-center justify-center">
                            <p class="text-xl" style="border-bottom: 1px solid black; margin-bottom:10px;">LOAN OF CERTIFICATION</p>
                          </section>
                          <section class="py-1 flex flex-col gap-4 text-md">
                            ...
                            <article class="text-lg" id="content">
                                <p contentEditable="true">
                                    THIS IS TO CERTIFY that <b>{{ $loan->firstname.' '.$loan->middlename.' '.$loan->lastname }}</b> , of legal age, single, Filipino, is a bona fide resident of <span style="text-transform: uppercase;">{{  $loan->district->name }}  , {{ $loan->district->barangay->municipal->name  }} {{ $loan->district->barangay->municipal->province->name  }}</span>, who is known to me personally, as a law-abiding citizen.
                                </p>
                                <br>
                                <p contentEditable="true">
                                    THIS CERTIFIES FURTHER that the above-named person is currently working as a Junker.
                                </p>
                                <br>
                                <p contentEditable="true">
                                    THIS CERTIFICATION is issued upon the formal request of the above-named person in processing pertinent papers to support his LOAN APPLICATION.
                                </p>
                                <br>
                                <p contentEditable="true">
                                    Given  this  8th   day  of  January,  2024  at   Barangay {{ $loan->district->barangay->name  }} , {{ $loan->district->barangay->municipal->name  }} {{ $loan->district->barangay->municipal->province->name  }}
                                </p>
                                <br>
                                <p contentEditable="true">
                          ...
                          <section class="py-2 items-end flex flex-row justify-between text-md pt-2">
                            <div class="flex flex-col items-center">
                                <p style="border-bottom: 1px solid black; font-weight:bold;" contentEditable="true">[ Kagawad Name ]</p>
                        ...
                        <section class="pb-2 pt-[40px]  items-start flex flex-col text-md" style="font-family:pacifico;">
                            ...
                            <div class="flex flex-col items-center">
                                <span class="w-full flex flex-row">
                                    ...
                                    <span>: <span contentEditable="true" id="amountPaid" data-user="{{ $loan->id }}">P 500 pesos</span></span>
                                </span>
                                <span class="w-full flex flex-row">
                                ...
                                <span class="w-full flex flex-row">
                                    ...
                                    <span>: <span contentEditable="true">Barangay {{ $loan->district->barangay->name  }} , {{ $loan->district->barangay->municipal->name  }} {{ $loan->district->barangay->municipal->province->name  }}</span></span>
                                </span>
                            </div>
                        </section>
                     </div>
        ...
        {{-- <a class="py-2 px-3 bg-blue-400 text-2xl rounded-md" href="{{ route('admin.indigency.show',$loan->id) }}">print</a> --}}
        <div class="flex flex-col gap-5">
            <button class="py-2 px-3 bg-green-600 text-white rounded-md shadow-md" id="{{ $loan->id }}" onclick="printDiv(this)">Print</button>
