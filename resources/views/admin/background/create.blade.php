...
<div class="relative overflow-x-auto sm:rounded-lg">
    <div class="max-w-[70%]">
        <form  method="POST" enctype="multipart/form-data" action="{{ route('admin.background.store') }}" class="bg-white rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            <div class=" flex gap-2">
                <div class="mb-4 flex-1">
                  @if ($background!=null)
                      @if ($background->name)
                      <img class="p-10 w-full rounded-t-lg h-96 md:h-auto md:w-48 md:rounded-none md:rounded-l-lg" style="width: 600px; height:600px;" src="{{ asset('images/background/'.$background->name) }}" alt="">
                      @else
              ...
              <div class="mb-4 flex-1">
                <div class="mb-4 flex-1">
                    ...
                    <input name="background" accept="image/jpeg, image/png"  class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="file">
                    <small>@error('background') <p class="text-red-500">{{ $message }}</p>  @enderror</small>
                </div>
                <div class="mb-4 flex-1">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="username">
                    Background Title
