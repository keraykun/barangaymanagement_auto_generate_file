            ...
            <section style="display:flex; flex-direction:column; flex-wrap:wrap; justify-content:center; align-items:center;">
              <article class="barangay-choose" style="font-size: 2rem;">Province of  {{ Str::ucfirst( $province->name) }}</article>
               <article class="flex justify-between items-center gap-2">
                <div>
                    <a class="bg-green-700 p-4 text-white" href="{{ route('admin.province.index') }}"><i class="fa fa-arrow-left"></i></a>
                </div>
                ...
                <div>
                    ...
                    <form method="GET" class="bg-green-700 p-[14px]" action="{{ route('admin.municipal.create') }}">
                        @csrf
                        @method('PATCH')
                        <input type="hidden" name="province_id" value="{{ $province->id }}">
                        @if (auth()->user()->role=='admin')
                        <button class=" text-white" type="submit"><i class="fa fa-add"></i></button>
                        @endif
                     </form>
                    @endif
                </div>
                <div id="dropdown" class="z-10 hidden bg-white divide-y divide-gray-100  shadow w-44 dark:bg-gray-700">
                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                        ...
                        <a href="{{ route('admin.municipal.show',$municipal->id) }}" class="block px-4 py-2 text-lg hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                        <span>{{ $municipal->name }}</span>
                        @if (auth()->user()->role=='admin')
                        <a href="{{ route('admin.municipal.edit',$municipal->id) }}"><i class="fa fa-pencil"></i></a>
                        @endif
                        </a>
                    </li>
                    @endforeach
